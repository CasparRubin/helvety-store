...
import { stripe, getProductFromPriceId, isHandledWebhookEvent } from '@/lib/stripe'
...
async function handleSubscriptionUpsert(subscription: Stripe.Subscription) {
  ...
  if (!userId) {
    ...
    if (!profile) {
      logger.warn(`No user found for subscription ${subscription.id}, customer ${customerId}`)
      return
    }
...
async function upsertSubscription(subscription: Stripe.Subscription, userId: string) {
  ...
  const { error } = await supabase
    ...
    .upsert({
      ...
      product_id: productInfo?.productId || subscription.metadata?.product_id || 'unknown',
      tier_id: productInfo?.tierId || subscription.metadata?.tier_id || 'unknown',
  ...
  await supabase.from('subscription_events').insert({
    user_id: userId,
    event_type: subscription.status === 'active' ? 'subscription.created' : 'subscription.updated',
  ...
  logger.info(`Subscription ${subscription.id} upserted for user ${userId}, status: ${subscription.status}`)
}
...
async function handleSubscriptionDeleted(subscription: Stripe.Subscription) {
  ...
  if (error) {
    logger.error(`Failed to mark subscription ${subscription.id} as canceled:`, error)
    throw error
  }
...
async function handleInvoicePaid(invoice: Stripe.Invoice) {
  ...
  if (!sub) {
    logger.warn(`Subscription ${subscriptionId} not found for invoice ${invoice.id}`)
    return
  }
  ...
  logger.info(`Subscription ${subscriptionId} renewed via invoice ${invoice.id}`)
}
...
async function handleInvoicePaymentFailed(invoice: Stripe.Invoice) {
  ...
  if (!sub) {
    logger.warn(`Subscription ${subscriptionId} not found for failed invoice ${invoice.id}`)
    return
  }
