...
export function ProductCard({ product, className }: ProductCardProps) {
  ...
  const priceDisplay = formatStartingFrom(product.pricing, product.pricing.tiers[0]?.currency)

  return (
    <Card className={cn('group relative flex flex-col transition-all hover:shadow-md', className)}>
      <CardHeader>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                ...
                {product.status !== 'available' && <StatusBadge status={product.status} />}
              </div>
            </div>
          </div>
        </div>
      ...
      <CardFooter className="flex items-center justify-between gap-4">
        <div className="text-sm font-medium">
          {product.pricing.hasFreeTier ? (
            <span className="text-green-600 dark:text-green-400">Free to start</span>
          ) : (
